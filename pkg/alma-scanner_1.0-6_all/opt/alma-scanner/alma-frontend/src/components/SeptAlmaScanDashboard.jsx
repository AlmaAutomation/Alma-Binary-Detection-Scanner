...
// If you run CRA dev server on :3000 talking to :7072, set:
//   REACT_APP_API_BASE=http://127.0.0.1:7072
...
export default function AlmaScanDashboard() {
  ...
  const fetchScan = useCallback(async () => {
    ...
    try {
      ...
      const list = res.data.binaries || [];
      setBinaries(list);
      setBinaryCount(
        typeof res.data.binary_count === "number"
          ? res.data.binary_count
          : list.length
      );
      setTotalSeen(
        typeof res.data.total_seen === "number" ? res.data.total_seen : null
      );
      fetchMetrics();
    } catch (err) {
          ...
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end">
              ...
              <div className="flex items-center gap-1 mr-2 text-xs">
                ...
                <span className="px-2 py-1 rounded bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-gray-200">
                  unknown: {archSummary.unk}
                </span>
                <span className="px-2 py-1 rounded bg-slate-200 text-slate-800 dark:bg-slate-700 dark:text-slate-200">
